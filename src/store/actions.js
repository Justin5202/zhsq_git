...
function checkData(data, commit, first, type) {
    ...
    let falseLength = 0
    let isGo = true
    if (!first && cur.isActive && cur.children && cur.children.length > 0) {
        cur.children.map(v => {
            if (!v.isActive) {
                falseLength += 1
            }
...
function addIsActive(data) {
    data.isActive = false
    let temp = []
    let menu = state.areaInfoList.slice()
    temp.push(data)
...
export const getSearchParams = function ({ dispatch, commit, state }, { typeParams, params }) {
    let data = Object.assign(
        ...
        typeParams,
        params,
        state.areaList[state.areaList.length - 1])
